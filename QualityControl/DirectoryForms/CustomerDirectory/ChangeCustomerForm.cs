...
namespace QualityControl_Server.Forms.CustomerDirectory
{
    public partial class ChangeCustomerForm : ChangeForm
    {
        ...
        public ChangeCustomerForm(DirectoryForm parent, BllCustomer oldCustomer, IUnitOfWork uow) : base(parent)
        {
            ...
            {
                foreach (var Contract in oldCustomer.ContractLib.Entities)
                {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (Contract != null)
            {
                oldCustomer.ContractLib.Entities.Add(Contract);
                listBox1.Items.Add(Contract.Name);
            }
        }
        ...
        {
            oldCustomer.ContractLib.Entities.RemoveAt(listBox1.SelectedIndex);
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            if (i != -1)
            {
                ChangeContractForm ContractForm = new ChangeContractForm(oldCustomer.ContractLib.Entities[i], uow);
                ContractForm.ShowDialog(this);

                BllContract Contract = ContractForm.oldContract;
                if (Contract != null)
                {
                    oldCustomer.ContractLib.Entities[i] = Contract;
